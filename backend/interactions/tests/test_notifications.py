...
class NotificationTests(APITestCase):
    ...
    def setUpTestData(cls):
        cls.alice = User.objects.create_user(
            username="alice", password="x", email="alice@t.com"
        )
        cls.bob = User.objects.create_user(
            username="bob", password="x", email="bob@t.com"
        )
    ...
    def test_filtro_lida_false(self):
        n1 = Notification.objects.create(
            recipient=self.alice, actor=self.bob, tipo="follow", lida=True
        )
        Notification.objects.create(
            recipient=self.alice, actor=self.bob, tipo="follow", lida=False
        )
    ...
    def test_marca_como_lida(self):
        n = Notification.objects.create(
            recipient=self.alice, actor=self.bob, tipo="follow"
        )
        self.client.force_authenticate(self.alice)
        resp = self.client.patch(
            reverse("notification-detail", args=[n.id]), {"lida": True}, format="json"
        )
    ...
    def test_nao_marca_notificacao_alheia(self):
        n = Notification.objects.create(
            recipient=self.bob, actor=self.alice, tipo="follow"
        )
        self.client.force_authenticate(self.alice)
        resp = self.client.patch(
            reverse("notification-detail", args=[n.id]), {"lida": True}, format="json"
    ...
    def test_marcar_tudo_lido(self):
        ...
        self.assertEqual(resp.status_code, 200)
        self.assertEqual(
            Notification.objects.filter(recipient=self.alice, lida=False).count(), 0
        )
